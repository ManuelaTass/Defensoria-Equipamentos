            ...
            <Table>
              <TableHeader className="bg-secondary/30">
                <TableRow>
                  ...
                  <TableHead className="font-semibold text-foreground text-center">Equipe / Equip.</TableHead>
                  <TableHead className="text-right font-semibold text-foreground">Detalhes</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {pagedEvents.map((evt) => (
                  <TableRow key={evt.id} className="hover:bg-secondary/10 transition-colors">
                    <TableCell className="font-medium max-w-[180px]">
                      <span className="line-clamp-2 leading-tight">{evt.name.trim()}</span>
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                      <div className="text-sm">
                        <div>{format(new Date(evt.startDate), "dd/MM/yy HH:mm", { locale: ptBR })}</div>
                        <div className="text-muted-foreground text-xs mt-0.5">até {format(new Date(evt.endDate), "dd/MM/yy HH:mm", { locale: ptBR })}</div>
                      </div>
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell className="text-center">
                      <div className="flex items-center justify-center gap-3 text-xs text-muted-foreground">
                        <span className="flex items-center gap-1">
                          ...
                          {(evt as any).technicianCount ?? 0}
                        </span>
                        <span className="flex items-center gap-1">
                          <Package className="h-3 w-3" />
                          {(evt as any).equipmentCount ?? 0}
                        </span>
                      </div>
                    </TableCell>
